...
if (jQuery.expr.createPseudo) {
  jQuery.expr[":"].icontains = jQuery.expr.createPseudo(function(arg) {
    return function(elem) {
      ...
      return text.indexOf(arg.toLowerCase()) >= 0;
    };
  });
} else {
  jQuery.extend((jQuery.expr[':'].icontains = function(a, i, m) {
    var text;
    return text = (a.textContent || a.innerText || "").toLowerCase().replace(/\xA0/g, " ").indexOf((m[3] || "").toLowerCase()) >= 0;
  }));
}
...
Range.nodeFromXPath = function(xpath, root, matchText, offset) {
  var customResolver, evaluateXPath, found, index, myindex, n, namespace, node, segment, text, xp, _i, _len;
  if (root == null) {
    root = document;
  }
  if (matchText == null) {
...
Annotator.Plugin.Permissions = (function(_super) {
  ...
  Permissions.prototype.updateViewer = function(field, annotation, controls) {
    ...
    usertitle = annotation.user && annotation.user.name ? annotation.user.name : username;
...
Annotator.Plugin.Comment = (function(_super) {
  ...
  Comment.prototype.addReplyButton = function(viewer, annotations) {
    var dateString, div, idx, item, listing, published, replies, reply, replylist, username, usertitle, _i, _j, _len, _len1;
    ...
    for (idx = _i = 0, _len = listing.length; _i < _len; idx = ++_i) {
      ...
      if (replies.length > 0) {
        ...
        for (_j = 0, _len1 = replies.length; _j < _len1; _j++) {
          reply = replies[_j];
          usertitle = reply.user.name || reply.user;
          username = Util.userString(reply.user);
          published = new Date(reply.updated || reply.created);
...
Annotator.Erratum = (function(_super) {
  ...
  Erratum.prototype._setupComment = function(annotation) {
    var comment, dateString, div, erratum, icon, missing, published, quote, replies, reply, self, textString, userString, userTitle, where, _i, _len;
    self = this;
    ...
    published = new Date(annotation.updated || annotation.created);
    dateString = Util.dateString(published);
  ...
  Erratum.prototype._reloadComment = function(annotation) {
    var comment, self;
  ...
  Erratum.prototype.annotationDeleted = function(annotation) {
    ...
    comment.slideUp(function() {
      comment.remove();
      return self._setupComment(annotation);
    });
